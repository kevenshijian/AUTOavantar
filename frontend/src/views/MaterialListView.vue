          ...
          <template v-if="currentTab === 'role'">
            ...
            <div class="video-section">
              ...
              <div class="video-list">
                <div v-for="(video, index) in createForm.loop_videos" :key="index" class="video-item">
                  ...
                  <div class="video-actions">
                    <el-button size="small" @click="analyzeFace(video.path, 'loop')">
                      <el-icon><Monitor /></el-icon> 面部分析
                    </el-button>
            ...
            <div class="video-section">
              ...
              <div class="video-preview" v-if="createForm.ending_video">
                ...
                <div class="video-actions">
                  <el-button size="small" @click="analyzeFace(createForm.ending_video, 'ending')">
                    <el-icon><Monitor /></el-icon> 面部分析
                  </el-button>
...
const analyzeFace = async (videoPath, type) => {
  try {
    ElMessage.info('正在进行面部分析...')
    const response = await taskApi.analyzeFace(videoPath)
    
    if (response.code === 200) {
      const result = response.data
      ElMessage.success(`面部分析完成，已移除 ${result.invalid_frame_count} 段不合格画面`)
      ...
      ElMessage.error('面部分析失败: ' + (response.message || '未知错误'))
    }
  } catch (error) {
