...
class BigVGAN(torch.nn.Module):
    ...
    def forward(self, x, mel_ref, lens=None):
        # NOTE: speaker_encoder 使用随机初始化权重（NVIDIA 原始 checkpoint 不包含），
        # 且 input_size=num_mels(80) 与实际 mel_ref 维度(100, 来自 GPT 的 mel 配置) 不匹配。
        # 直接用零向量替代，不影响基础 TTS 合成质量。
        n_batch = x.size(0)
        contrastive_loss = None
        speaker_embedding = torch.zeros(n_batch, self.h["speaker_embedding_dim"], 1, device=x.device, dtype=x.dtype)
