...
class IndexTTS:
    def __init__(
        ...
        ckpt_state = vocoder_dict["generator"]

        # 将 checkpoint 权重加载到模型中，处理 shape mismatch
        # NVIDIA 原始 BigVGAN checkpoint 的 conv_pre 输入维度为 80，
        # 但 IndexTTS 扩展版 BigVGAN 的 conv_pre 输入维度为 gpt_dim（1280），
        # 同时新增了 speaker_encoder / cond_layer / conds 等子模块（checkpoint 中不存在）。
        model_state = self.bigvgan.state_dict()
        for key in ckpt_state:
            ...
            if ckpt_shape == model_shape:
                ...
                # shape mismatch — 使用 kaiming 初始化替代
                # 原因：conv_pre 的输入维度从 80(mel) 变为 1280(gpt_dim)，
                # 两种输入的语义完全不同，不能混用权重
                import math
                print(f">> [BigVGAN] shape mismatch for '{key}': "
                      f"ckpt={ckpt_shape} vs model={model_shape}, using kaiming init")
                if len(ckpt_shape) == 3 and key.startswith("conv_pre."):
                    # Conv1d weight: [out_ch, in_ch, kernel]
                    nn.init.kaiming_normal_(model_state[key], a=math.sqrt(5))
                # bias 或其他 shape mismatch 的键保持默认初始化
        self.bigvgan.load_state_dict(model_state)
    ...
    def infer_fast(self, audio_prompt, text, output_path, verbose=False, max_text_tokens_per_sentence=100,
        ...
        emo_vec = generation_kwargs.pop("emo_vec", None)
        use_speed = generation_kwargs.pop("use_speed", False)
        sampling_rate = 24000
        # lang = "EN"
        # lang = "ZH"
        wavs = []
